...
日常开发中，代码片段散落在各处（编辑器历史、笔记软件、聊天窗口），需要时难以快速检索；Markdown 笔记与 API 测试又依赖不同的工具，上下文频繁切换。受到 [Raycast](https://www.raycast.com/) 的快启搜索和 Notion 的简洁设计的启发，我希望做一款**All-in-One 的开发者效率工具**，把片段管理、笔记记录和 API 测试整合到同一个界面中，并通过课程项目实践 **Vue 3 + Electron 跨平台开发**的完整流程。
...
除本课程项目外，我还独立负责以下前端工程的全栈开发与持续维护：

| 项目 | 地址 | 技术栈 | 说明 |
|------|------|--------|------|
| **Shimen Stata** | https://simen.site/ | React + Vite | 学术数据分析工具，支持面板数据上传、显著回归结果筛选、Word 三线表与 Stata do 文件导出 |
| **RollingTheRock Blog** | https://rollingtherock.blog/ | Astro + Tailwind CSS | 技术博客，支持深色模式、RSS、移动端响应式菜单与 SEO 优化 |

这些项目的持续开发与部署经验，为 SnippetVault 的跨平台架构、性能优化和 UI 设计提供了实践基础。
